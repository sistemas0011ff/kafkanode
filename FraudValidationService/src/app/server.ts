...
import { TransactionCreatedSubscriberToken } from '../contexts/fraud-detection/application/event-handlers/TransactionCreatedSubscriber';
import { TransactionCreatedSubscriber } from '../contexts/fraud-detection/application/event-handlers/TransactionCreatedSubscriber';
import { KafkaService, KafkaServiceToken } from '../contexts/shared/infrastructure/kafka/KafkaService';
import { Kafka, logLevel } from 'kafkajs';
import { KafkaConsumerServiceToken } from '../contexts/fraud-detection/infraestructure/messaging/KafkaConsumerService';
import { KafkaConsumerService } from '../contexts/fraud-detection/infraestructure/messaging/KafkaConsumerService';
import { FraudValidationDomainServiceToken } from '../contexts/fraud-detection/domain/services/FraudValidationDomainService';
import { FraudValidationDomainService } from '../contexts/fraud-detection/domain/services/FraudValidationDomainService';
...
export class Server {
    ...
    constructor(appName: string, basePath: string, port: string, enviroment: string) {
        ...
        const kafkaTopic = process.env.KAFKA_TOPIC_CREATED;
        ...
        const kafkaService = Container.get<KafkaService>(KafkaServiceToken);
        kafkaService.connectProducer().then(() => {
        ...
        Container.set(TransactionCreatedSubscriberToken, new TransactionCreatedSubscriber(
            Container.get(KafkaServiceToken),
            Container.get(FraudValidationDomainServiceToken)
        ));
        ...
        return new Promise((resolve, reject) => {
            ...
            this.httpsServer.listen(this.port, () => {
                ...
                kafkaConsumerService.start()
                    ...
                    .catch(error => {
                        log.error("Error al iniciar KafkaConsumerService");
                    });
