...
export class KafkaConsumerService implements IKafkaConsumerService {
  ...
  constructor(
    ...
    this.topic ="transaction_status_updated"; 
    console.log(`VVVVVV KafkaConsumerService iniciado para el tópico: ${this.topic}`);
  }
  ...
  private transformToDomainEvent(kafkaMessage): TransactionUpdatedDomainEvent | null {
    try {
      const data = JSON.parse(kafkaMessage.value.toString());
      return TransactionUpdatedDomainEvent.fromPrimitives(data);
    } catch (error) {
